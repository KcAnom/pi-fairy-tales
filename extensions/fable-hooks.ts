/**
 * fable-hooks: guard rails + post-edit test runner.
 *
 ...
 * - plan mode enforcement: while fable-plan broadcasts active=true, mutating
 ...
 *
 * Rules stay active inside fable subagents — guard rails apply everywhere.
 */
import { existsSync, readFileSync } from "node:fs";
import { join, resolve } from "node:path";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { isToolCallEventType } from "@earendil-works/pi-coding-agent";
import { loadFableConfig, type FableConfig } from "../src/config.ts";
...
export default function (pi: ExtensionAPI) {
    ...
    if (isToolCallEventType("bash", event)) {
      ...
      if (verdict) {
        if (verdict.action === "block") {
          return { block: true, reason: `Blocked by fable rule: ${verdict.reason}` };
        }
        // confirm — fail-safe to block when headless
        if (!ctx.hasUI) {
          return { block: true, reason: `Blocked (needs confirmation, no UI): ${verdict.reason}` };
        }
        const ok = await ctx.ui.confirm("Fable guard", `${verdict.reason}\n\n${command}\n\nAllow?`);
      ...
      if (path) {
        ...
        if (verdict) {
          if (verdict.action === "block" || !ctx.hasUI) {
            return { block: true, reason: `Blocked by fable path rule: ${verdict.reason} (${abs})` };
          }
          const ok = await ctx.ui.confirm("Fable guard", `${verdict.reason}\n\n${abs}\n\nAllow?`);
          if (!ok) return { block: true, reason: `User denied: ${verdict.reason}` };
        }
  ...
  pi.on("turn_end", async (_event, ctx) => {
    ...
    if (ctx.hasUI) ctx.ui.setStatus("fable-tests", "⏳ tests");
    ...
    void (async () => {
      try {
        ...
        testRunning = false;
        if (ctx.hasUI) ctx.ui.setStatus("fable-tests", undefined);
      }
    })();
  });
}
