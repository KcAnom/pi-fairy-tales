...
- **Chain-facing quest surface** — the `quest` tool now exposes the durable-pipeline primitives: `dedupeKey` (idempotent enqueue), `chain` metadata (`{chain, runId, phase, workId}`), `retainUntilConsumed`, targeted runs (`run` with an `id`), and new `requeue`/`consume` actions. `QuestStore.release()`/`QuestRuntime.releaseClaim()` hand a claimed quest back without burning its retry budget. Built for pi-fairy-tales-chains 0.2.0 (JSON-authoritative chain state), which drives these from its `chain` tool.
- **Codebase intelligence** — new read-only `codebase_intel` tool over an incremental import-graph index: `deps` (what a file depends on, transitive with depth), `impact` (reverse traversal — everything that could break if a file changes), `hotspots` (fan-in/fan-out/size/git-churn ranking), and `status`. Parses with the TypeScript compiler API when available and a regex scanner otherwise; the cache lives outside analyzed projects at `~/.pi/agent/cache/code-intelligence/` (nothing is ever written into a repo). Kill switch: `"codeIntel": { "enabled": false }`.
